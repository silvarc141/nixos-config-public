{
  ...
  options.programs.thunar.enable = lib.mkEnableOption "thunar file manager";

  config = let
    package = with pkgs; (xfce.thunar.override {
      thunarPlugins = [
  ...
  in
    lib.mkIf config.programs.thunar.enable {
      custom.graphical.desktopAgnostic.extraCommands.fileManager = "${package}/bin/thunar";
      home.packages = [package];
      custom.graphical.defaultApps.fileManager = "thunar.desktop";
      ...
      xfconf.settings = {
        thunar = {
          ...
          misc-single-click-timeout = 0;
          misc-volume-management = false;
          last-image-preview-visible = true;
          misc-symbolic-icons-in-sidepane = true;
          last-separator-position = 166;
          last-location-bar = "ThunarLocationButtons";
          shortcuts-icon-size = "THUNAR_ICON_SIZE_16";
          last-details-view-visible-columns = "THUNAR_COLUMN_DATE_MODIFIED,THUNAR_COLUMN_NAME,THUNAR_COLUMN_SIZE";
          last-toolbar-items = "back:0,forward:0,open-parent:0,open-home:0,new-tab:0,new-window:0,toggle-split-view:0,undo:0,redo:0,zoom-out:0,zoom-in:0,zoom-reset:0,view-as-icons:0,view-as-detailed-list:0,search:0,view-as-compact-list:0,view-switcher:0,reload:0,location-bar:1,uca-action-1753574365446811-1:1,menu:1";
        };
      };
          ...
          ''
            ...
            <actions>
              <action>
               ...
               <submenu></submenu>
               <unique-id>1753574365446811-1</unique-id>
               <command>kitty -d %f</command>
               <description>Open terminal in the directory</description>
               <range></range>
               <patterns>*</patterns>
               <startup-notify/>
               <directories/>
      ...
      #       (gtk_accel_path "<Actions>/ThunarActionManager/trash-delete" "x")
      #
      ...
      #
      #       (gtk_accel_path "<Actions>/ThunarStandardView/forward" "<Primary>i")
      #       (gtk_accel_path "<Actions>/ThunarStandardView/back" "<Primary>o")
      #     '';
      # };
