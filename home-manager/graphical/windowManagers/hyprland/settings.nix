    ...
    wayland.windowManager.hyprland = {
      settings = {
        ...
        cursor = {
          ...
          inactive_timeout = "2";
          # I'd like it to be true but when it is:
          # - Focus is sometimes completely lost when mouse moves over an exclusive layer such as waybar. It is regained when spawning a new window, but after closing that window it is lost completely again, thus forcing to move the mouse in order to focus that window using keyboard.
      ...
      in [
        ...
        # # pin
        # "SUPER, P, ${snippet ''sys clients | where pinned | get pid | each { hyprctl dispatch settiled $"pid:($in)" }''}"
