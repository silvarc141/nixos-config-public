{
  ...
  nixosConfigDirRelative = "nixos-config";
...
in {
  ...
  config = lib.mkIf (config.services.sync-nixos-config.enable && config.custom.secrets.enable) {
    sops.secrets.ssh-nixos-config-rw = {};

    programs.ssh.matchBlocks = {
      "nixos-config.git" = {
        hostname = "github.com";
        user = "git";
        identityFile = config.sops.secrets.ssh-nixos-config-rw.path;
      };
    };
    ...
    custom.ephemeral.data = {
      directories = [nixosConfigDirRelative];
    };
  };
}
